...
import com.example.guitartuner.ui.settings.components.TuningControls.SaveTuningDialog
...
import com.example.guitartuner.ui.settings.components.TuningControls.TuningSettingsItemSwippable
...
private fun SettingsTuningsScreenContent(
    ...
    if (showSaveDialog) currentTuningState?.let {
        val defaultName = "${it.instrumentName} - ${it.notesList}"
        SaveTuningDialog(
            ...
            onSaveTuning(name.ifBlank { defaultName })
        }
    }
    ...
    LazyColumn {
        ...
        items(count = tuningSets.size, key = { tuningSets[it].tuningId }) { index ->
            val tuning = tuningSets[index]
            TuningSettingsItemSwippable(
                tuning = tuning,
                onSelect = onSelectTuning,
                onFavSelect = onToggleFavorite,
                onRemove = onRemoveTuning
            )
        }
    }
}
